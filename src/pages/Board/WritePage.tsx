...
const WritePage: React.FC<WritePageProps> = ({ onPostSubmit }) => {
  ...
  const [nickname, setNickname] = useState(''); // Add the nickname state
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (content.trim() === '' || nickname.trim() === '') { // Check if the nickname is empty
    ...
    try {
      ...
      const newPost = {
        mbti: 'ISFJ',
        nickname: nickname, // Use the nickname state here
        content: content,
      };
      ...
      <div className="max-w-md w-4/5 mx-auto p-6 bg-white rounded-lg shadow-md">
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          <input
            type="text"
            value={nickname}
            onChange={handleNicknameChange} // Handle nickname change
